...
#### configs, would be passed in other ways
#### but not by import method as previously done

LINK_SPEEDS = [("^TwentyGigE*", "20"),
               ("^FortyGig*", "40"),
               ("^Ten-GigabitEthernet*", "10"),
               ("^GigabitEthernet*", "1")]
sysOIDs = [('SNMPv2-MIB', 'sysName', 0),('SNMPv2-MIB', 'sysServices')]
...
class NetworkTopology:
    ...
    def classifyDeviceHostnameToGroup(self, s_sysDescr:str):
        if s_sysDescr:

            descr_num = int(s_sysDescr)
            if descr_num == 
            OSI = 
        return 1 # defualt for normal computer client
    ...
    def addDevice(self, s_sysName:str, s_sysDescr:str):
        # check if device is already added
        if self.isDeviceDefined(s_sysName):
            return False

        # Device Type classification
        group_number = self.classifyDeviceHostnameToGroup(s_sysDescr)
        if not group_number:
            logging.error("Cannot decide device type of -- sysName:{}, s_sysDescr:{}".format(s_sysName,s_sysDescr))
            return False
        image_name = ["*","default.png", "AC.png","DS.png","L2.png","L3.png"][group_number]

        # default device setting = {"id": s_device_name, "group": 1, "image": default.png}

        self.visualization_dict['nodes'].append({"id": s_sysName, "group": group_number, "image": image_name})
        self.interfaces_dict[s_device_name] = []
        self.neighborships[s_device_name] = []
        return True

    def isDeviceDefined(self,s_device_name):
        return next((device for device in self.visualization_dict['nodes'] 
            if device['id'] == s_device_name),False)


    #### extended functions for more information ####
    """ temp disable
    ...
    def getDeviceInterfaceUtilizations(self,deviceid,interface_name):
        ...
        search_result = next((interface for interface in self.interface_stats[deviceid]['interfaces'] 
            if interface['ifDescr'] == interface_name ), [])
        
        if 'utilization' in search_result:
            return search_result['utilization']
        return 0
    """
    ...
    def addDeviceInterface(self, s_device_name, 
        ...
        if str(ifType) in IGNORED_IFTYPES : return
    ...
    def addInterfaceStats(self,
                          deviceid, INDEX, ifDescr, ifType,
                          ifOperStatus, ifHCInOctets,
        ...
        if deviceid not in self.interface_stats:
            ...
            # Here we have a found device and found non-empty interface stats
                ...
                last_ifHCInOctets = self.my_int(device_interface_stats['last_ifHCInOctets'])
                last_ifHCOutOctets = self.my_int(device_interface_stats['last_ifHCOutOctets'])
                last_timestamp = self.my_int(device_interface_stats["last_timestamp"])

                # IF NEW COUNTERS ARE SMALLER THAN PREVIOUS, PROBABLY A COUNTER WRAP AND WE SKIP
                if self.my_int(ifHCInOctets) > last_ifHCInOctets and self.my_int(ifHCOutOctets) > last_ifHCOutOctets:

                    delta_time = timestamp - last_timestamp
                    delta_ifHCInOctets = self.my_int(ifHCInOctets) - last_ifHCInOctets
                    delta_ifHCOutOctets = self.my_int(ifHCOutOctets) - last_ifHCOutOctets
                    # This means our speed on In was :

                    InSpeed = self.my_int(delta_ifHCInOctets * 8 / 1024 / 1024 / delta_time)
                    OutSpeed = self.my_int(delta_ifHCOutOctets * 8 / 1024 / 1024 / delta_time)

                    # Utilization
                    utilization = self.my_int(max(InSpeed, OutSpeed) / self.my_int(ifHighSpeed) * 100)
...
if __name__ == "__main__":
    nodes = []
    candidate = {"id": "deviceA", "group": "1", "image": "default.png"}
    nodes.append(candidate)
    candidate = {"id": "deviceB", "group": "2", "image": "default.png"}
    nodes.append(candidate)
    ...
    with open('samplegraph.json', 'w') as outfile:
        json.dump(visualization_dict, outfile, sort_keys=True, indent=4)
        print("")
        print("JSON printed into samplegraph.json")
